/**
   ...
   * Follow https://github.com/zerodev
*/
...
async function startZero() {
    ...
    // anticall auto block
    zero.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let pa7rick = await zero.sendContact(callerId, global.owner)
    zero.sendMessage(callerId, { text: `*Sistem otomatis block!*\n*Jangan menelpon bot*!\n*Silahkan Hubungi Owner Untuk Dibuka !*`}, { quoted : pa7rick })
    await sleep(8000)
    await zero.updateBlockStatus(callerId, "block")
    }
    })
    ...
    // Get Profile Picture Group
       try {
       ppgc = await zero.profilePictureUrl(pea[0].id, 'image')
       } catch {
       ppgc = 'https://shortlink.hisokaarridho.my.id/rg1oT'
       }
       let wm_zerodev = { url : ppgc }
       if (pea[0].announce == true) {
       zero.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message by zero Dev`, wm_zerodev, [])
       } else if(pea[0].announce == false) {
       zero.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message by zero Dev`, wm_zerodev, [])
       } else if (pea[0].restrict == true) {
       zero.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message by zero Dev`, wm_zerodev, [])
       } else if (pea[0].restrict == false) {
       zero.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message by zero Dev`, wm_zerodev, [])
       } else {
       zero.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup Subject telah diganti menjadi *${pea[0].subject}*`, `Group Settings Change Message by zero Dev`, wm_zerodev, [])
     }
    })
        ...
        try {
         ...
         const reSize = async(buffer, ukur1, ukur2) => {
             ...
             resolve(ab)
                ...
                if (anu.action == 'add') {
                    zero.sendMessage(anu.id, { caption: teks1, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butwel, footer: nyoutube, mentions: [num] })
                } else if (anu.action == 'remove') {
                    zero.sendMessage(anu.id, { caption: teks2, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butleav, footer: nyoutube, mentions: [num] })
                } else if (anu.action == 'promote') {
                    zero.sendMessage(anu.id, { caption: teks3, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butselamat, footer: nyoutube, mentions: [num] })
                } else if (anu.action == 'demote') {
                    zero.sendMessage(anu.id, { caption: teks4, location: { jpegThumbnail: await reSize(ppuser, 100, 100)}, buttons: butsebar, footer: nyoutube, mentions: [num] })
              }
            }
        } catch (err) {
            console.log(err)
    ...
    /** Send List Messaage
      ...
      */
      ...
      let bb = await zero.reSize(lok, 300, 300)
     ...
     * @returns
     */
    zero.send5ButImg = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: zero.waUploadToServer })
        ...
        imageMessage: message.imageMessage,
            ...
            zero.relayMessage(jid, template.message, { messageId: template.key.id })
    }
     ...
     * @returns
     */
    zero.send5ButVid = async (jid , text = '' , footer = '', vid, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: vid }, { upload: zero.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
     ...
     * @returns
     */
    zero.send5ButGif = async (jid , text = '' , footer = '', gif, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ video: gif, gifPlayback: true }, { upload: zero.waUploadToServer })
        var template = generateWAMessageFromContent(jid, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
        videoMessage: message.videoMessage,
     ...
     * @returns 
     */
    zero.sendText = (jid, text, quoted = '', options) => zero.sendMessage(jid, { text: text, ...options }, { quoted })
     ...
     * @returns 
     */
    zero.sendTextWithMentions = async (jid, text, quoted, options = {}) => zero.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
